...
Function Installation_interface_UI
{
	...
	Function Install_Init_Disk_To
	{
		switch ($UI_Main_Install_To.SelectedItem.Path) {
			"AutoSelectDisk" {}
			"Desktop" {
				Save_Dynamic -regkey "LXPs\Get" -name "InstlTo" -value "Desktop" -String
				return Join-Path -Path $([Environment]::GetFolderPath("Desktop")) -ChildPath $Default_directory_name
			}
			"Download" {
				Save_Dynamic -regkey "LXPs\Get" -name "InstlTo" -value "Download" -String
			...
			"Documents" {
				Save_Dynamic -regkey "LXPs\Get" -name "InstlTo" -value "Documents" -String
			...
			default {
				if (Get-ItemProperty -Path "HKCU:\SOFTWARE\Yi\LXPs\Get" -Name "Instl_To_Custom" -ErrorAction SilentlyContinue) {
					...
					Save_Dynamic -regkey "LXPs\Get" -name "InstlTo" -value $GetNewInstallTo -String
		...
		Save_Dynamic -regkey "LXPs\Get" -name "InstlTo" -value "AutoSelectDisk" -String
		$ExcludeDisk = @(
		...
		add_Click      = {
			...
			if ($FolderBrowser.ShowDialog() -eq "OK") {
				if (Test_Available_Disk -Path $FolderBrowser.SelectedPath) {
					...
					Save_Dynamic -regkey "LXPs\Get" -name "Instl_To_Custom" -value $UI_Main_Save_To_Path.Text -String
