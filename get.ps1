...
$AvailableLanguages = @(
	...
	@{
		...
		Language = @{
			FontsUI                  = "Segoe UI"
			Get                      = "Eskuratu Windows Tokiko Hizkuntza Esperientzia Paketeak (LXPs) deskargatzailea linean"
	...
	@{
		...
		Language = @{
			FontsUI                  = "Segoe UI"
			Get                      = "Obtén o descargador de paquetes de experiencia de idioma local de Windows (LXPs) en liña"
	...
	@{
		...
		Language = @{
			FontsUI                  = "Segoe UI"
			Get                      = "Dapatkan Pengunduh Paket Pengalaman Bahasa Lokal (LXPs) Windows Online"
	...
	@{
		...
		Language = @{
			FontsUI                  = "Segoe UI"
			Get                      = "Tải xuống trực tuyến Trình tải xuống gói trải nghiệm ngôn ngữ địa phương (LXPs) của Windows"
...
Function Installation_interface_UI
{
		...
		switch ($UI_Main_Install_To.SelectedItem.Path) {
			...
			default {
				if (Get-ItemProperty -Path "HKCU:\SOFTWARE\Yi\LXPs\Get" -Name "Instl_To_Custom" -ErrorAction SilentlyContinue) {
					$GetNewInstallTo = Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\Yi\LXPs\Get" -Name "Instl_To_Custom"
	...
	if (Get-ItemProperty -Path "HKCU:\SOFTWARE\Yi\LXPs\Get" -Name "InstlTo" -ErrorAction SilentlyContinue) {
		$GetNewInstallTo = Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\Yi\LXPs\Get" -Name "InstlTo"
...
#>
if ($Reset) {
	$Path = "HKCU:\SOFTWARE\Yi\LXPs\Get"
