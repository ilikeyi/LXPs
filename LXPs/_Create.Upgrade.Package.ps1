...
<#
  ...
  .压缩包保存到
#>
$UpdateSaveTo = "$([Environment]::GetFolderPath("Desktop"))\LXPs.Upgrade.Package"

<#
	.Archive temporary directory
	.压缩包临时目录
#>
$TempFolderUpdate = "$([Environment]::GetFolderPath("MyDocuments"))\Temp.LXPs.Upgrade.Package"
...
Function Update_Create_Process
{
	ForEach ($item in $BuildTypeUp) {
		...
		Update_Create_Version -SaveTo "$TempFolderUpdate" -CurrentVersion (Get-Module -Name LXPs).Version.ToString() -LowVer $((Get-Module -Name LXPs).PrivateData.PSData.MinimumVersion)
	}
}
