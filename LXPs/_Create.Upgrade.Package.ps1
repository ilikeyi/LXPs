...
Function Update_Create_UI
{
			...
			if ($UI_Main_Create_ASC.Enabled) {
				if ($UI_Main_Create_ASC.Checked) {
					if ([string]::IsNullOrEmpty($UI_Main_Create_ASCSign.Text)) {
						...
						Save_Dynamic -regkey "LXPs" -name "PGP" -value $UI_Main_Create_ASCSign.Text -String
	...
	if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$((Get-Module -Name LXPs).Author)\LXPs" -Name "PGP" -ErrorAction SilentlyContinue) {
		$UI_Main_Create_ASCSign.Text = Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$((Get-Module -Name LXPs).Author)\LXPs" -Name "PGP" -ErrorAction SilentlyContinue
	}
