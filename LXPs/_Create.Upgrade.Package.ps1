...
<#
  ...
  .压缩包保存到
#>
$UpdateSaveTo = Join-Path -Path ([Environment]::GetFolderPath("Desktop")) -ChildPath "Upgrade.Package\LXPs"
...
Function Update_Create_UI
{
	...
	$UI_Main_Create_ASC = New-Object System.Windows.Forms.CheckBox -Property @{
		...
		add_Click      = {
			if ($UI_Main_Create_ASC.Checked) {
				...
				Save_Dynamic -regkey "LXPs\GPG" -name "IsPGP" -value "True"
			} else {
				$UI_Main_Create_ASC_Panel.Enabled = $False
				Save_Dynamic -regkey "LXPs\GPG" -name "IsPGP" -value "False"
			}
		}
	}
			...
			if ($UI_Main_Create_ASC.Enabled) {
				if ($UI_Main_Create_ASC.Checked) {
					if ([string]::IsNullOrEmpty($UI_Main_Create_ASCSign.Text)) {
						...
						Save_Dynamic -regkey "LXPs\GPG" -name "PGP" -value $UI_Main_Create_ASCSign.Text
	...
	if (Test-Path -Path $Verify_Install_Path -PathType leaf) {
		...
		if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$($Global:Author)\LXPs\GPG" -Name "IsPGP" -ErrorAction SilentlyContinue) {
			switch (Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$($Global:Author)\LXPs\GPG" -Name "IsPGP" -ErrorAction SilentlyContinue) {
				"True" {
