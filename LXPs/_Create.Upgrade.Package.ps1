...
Remove-Module -Name LXPs -Force -ErrorAction Ignore | Out-Null
...
<#
  ...
  .压缩包保存到
#>
$UpdateSaveTo = Join-Path -Path ([Environment]::GetFolderPath("Desktop")) -ChildPath "LXPs.Upgrade.Package"

<#
	.Archive temporary directory
	.压缩包临时目录
#>
$TempFolderUpdate = Join-Path -Path ([Environment]::GetFolderPath("MyDocuments")) -ChildPath "Temp.LXPs.Upgrade.Package"
...
Function Update_Create_UI
{
	...
	$IconYi = "$($PSScriptRoot)\Modules\$((Get-Module -Name LXPs).Version.ToString())\Assets\icon\Yi.ico"
	...
	$GUIUpdateLowVersion = New-Object system.Windows.Forms.Label -Property @{
		...
		Height         = 30
		Width          = 390
		Text           = "$($lang.UpdateLow) $((Get-Module -Name LXPs).PrivateData.PSData.MinimumVersion)"
	}
	...
	$UI_Main_Create_ASC = New-Object System.Windows.Forms.CheckBox -Property @{
		...
		add_Click      = {
			if ($UI_Main_Create_ASC.Checked) {
				...
				Save_Dynamic -regkey "LXPs" -name "IsPGP" -value "True"
			} else {
				$UI_Main_Create_ASC_Panel.Enabled = $False
				Save_Dynamic -regkey "LXPs" -name "IsPGP" -value "False"
			}
		}
	}
			...
			if ($UI_Main_Create_ASC.Enabled) {
				if ($UI_Main_Create_ASC.Checked) {
					if ([string]::IsNullOrEmpty($UI_Main_Create_ASCSign.Text)) {
						...
						Save_Dynamic -regkey "LXPs" -name "PGP" -value $UI_Main_Create_ASCSign.Text
	...
	if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$($Global:Author)\LXPs" -Name "PGP" -ErrorAction SilentlyContinue) {
		$UI_Main_Create_ASCSign.Text = Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$($Global:Author)\LXPs" -Name "PGP" -ErrorAction SilentlyContinue
	}
	...
	if (Test-Path -Path $Verify_Install_Path -PathType leaf) {
		...
		if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$($Global:Author)\LXPs" -Name "IsPGP" -ErrorAction SilentlyContinue) {
			switch (Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$($Global:Author)\LXPs" -Name "IsPGP" -ErrorAction SilentlyContinue) {
				"True" {
...
Function Update_Create_Process
{
	...
	ForEach ($item in $BuildTypeUp) {
		...
		Update_Create_Version -SaveTo $TempFolderUpdate -buildstring $((Get-Module -Name LXPs).PrivateData.PSData.Buildstring) -CurrentVersion (Get-Module -Name LXPs).Version.ToString() -LowVer $((Get-Module -Name LXPs).PrivateData.PSData.MinimumVersion)
	}
}
