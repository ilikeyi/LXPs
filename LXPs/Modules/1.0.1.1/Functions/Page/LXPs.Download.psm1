...
Function LXPs_Download
{
			...
			if ($_ -is [System.Windows.Forms.RadioButton]) {
				if ($_.Enabled) {
					if ($_.Checked) {
						...
						Save_Dynamic -regkey "LXPs" -name "Channel" -value $_.Tag -String
					}
				}
			}
		}
	...
	if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$((Get-Module -Name LXPs).Author)\LXPs" -Name "Channel" -ErrorAction SilentlyContinue) {
		$GetSaveChannel = Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$((Get-Module -Name LXPs).Author)\LXPs" -Name "Channel" -ErrorAction SilentlyContinue
	} else {
		Save_Dynamic -regkey "LXPs" -name "Channel" -value "Retail" -String
