...
Function Prerequisite
{
	...
	if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$($Global:Author)\LXPs\Update" -Name "IsUpdate_Clean" -ErrorAction SilentlyContinue) {
		$GetOldVersion = Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$($Global:Author)\LXPs\Update" -Name "IsUpdate_Clean" -ErrorAction SilentlyContinue
		$SaveCurrentVersion = (Get-Module -Name LXPs).Version.ToString()
		...
		if ($GetOldVersion -eq $SaveCurrentVersion) {
			...
			Remove-ItemProperty -Path "HKCU:\SOFTWARE\$($Global:Author)\LXPs\Update" -Name "IsUpdate_Clean" -Force -ErrorAction SilentlyContinue | out-null
	...
	Write-Host "  $('-' * 80)"
	if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$($Global:Author)\LXPs\Update" -Name "IsAutoUpdate" -ErrorAction SilentlyContinue) {
		switch (Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$($Global:Author)\LXPs\Update" -Name "IsAutoUpdate" -ErrorAction SilentlyContinue) {
			"True"  {
				$SIPPS = Join-path -Path $PSScriptRoot -ChildPath "..\..\..\..\..\LXPs.ps1"
				$arguments = @(
					"-ExecutionPolicy",
					"ByPass",
					"-File",
				...
				if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$($Global:Author)\LXPs\Update" -Name "IsAutoUpdateNew" -ErrorAction SilentlyContinue) {
					...
					Save_Dynamic -regkey "LXPs\Update" -name "IsAutoUpdateNew" -value "True"
				}

				<#
					.自动检查更新间隔小时
				#>
				if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$($Global:Author)\LXPs\Update" -Name "AutoCheckUpdate_Hours" -ErrorAction SilentlyContinue) {
					$WaitHoursTime = Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$($Global:Author)\LXPs\Update" -Name "AutoCheckUpdate_Hours" -ErrorAction SilentlyContinue
				} else {
					$WaitHoursTime = "2"
					Save_Dynamic -regkey "LXPs\Update" -name "AutoCheckUpdate_Hours" -value "2"
				}
				...
				<#
					.判断是否有上次执行时间
				#>
				if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$($Global:Author)\LXPs\Update" -Name "Last_Auto_Update_Time" -ErrorAction SilentlyContinue) {
					$GetLastAutoUpdateTime = Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$($Global:Author)\LXPs\Update" -Name "Last_Auto_Update_Time" -ErrorAction SilentlyContinue
...
#					Write-Host "  Time: $($timeDiff.TotalSeconds) Seconds"
					...
					if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$($Global:Author)\LXPs\Update" -Name "Auto_Update_Last_status" -ErrorAction SilentlyContinue) {
						$GetAuto_Update_Last_status = Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$($Global:Author)\LXPs\Update" -Name "Auto_Update_Last_status" -ErrorAction SilentlyContinue
					} else {
		...
		Save_Dynamic -regkey "LXPs\Update" -name "IsAutoUpdate" -value "True"
	}
