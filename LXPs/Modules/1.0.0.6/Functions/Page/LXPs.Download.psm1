...
Function LXPs_Download_Licence_Process
{
	...
	$Region = Language_Region
	ForEach ($itemRegion in $Region) {
		$TempNewFileFolderPath = "$($FolderDirect)$($itemRegion.Region)"
		$TempNewFileFullPath = "$($FolderDirect)$($itemRegion.Region)\LanguageExperiencePack.$($itemRegion.Region).Neutral.appx"
	...
	if ($Script:Queue_Language_Download_Select.Count -gt 0) {
		...
		ForEach ($itemRegion in $Region) {
			if ($Script:Queue_Language_Download_Select -Contains $itemRegion.Region) {
				$NewFolder = "$($PSScriptRoot)\..\..\..\..\Download\$($Script:Version)\LocalExperiencePack\$($itemRegion.Region)"
				Check_Folder -chkpath $NewFolder
				$NewFolder = Convert-Path -Path $NewFolder -ErrorAction SilentlyContinue

				write-host "   $($itemRegion.Region.PadRight(45))$($itemRegion.Name)"
