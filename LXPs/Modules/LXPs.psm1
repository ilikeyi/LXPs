	...
	if ($Reset)
	{
		...
		if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$($Global:UniqueID)\LXPs" -Name "LanguagePrompt" -ErrorAction SilentlyContinue) {
			...
			if ($GetLanguagePrompt -eq "True") {
				if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$($Global:UniqueID)\LXPs" -Name "Language" -ErrorAction SilentlyContinue) {
					$GetLanguage = Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$($Global:UniqueID)\LXPs" -Name "Language"
...
{
	$Path = "HKCU:\SOFTWARE\$($Global:UniqueID)\LXPs"
	...
	if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$($Global:UniqueID)\LXPs" -Name "Language" -ErrorAction SilentlyContinue) {
		$FlagsDefaultLanguage = Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$($Global:UniqueID)\LXPs" -Name "Language"
	} else {
...
Function Language_Change
{
	...
	if (Test-Path "$($PSScriptRoot)\langpacks\$($lang)\Lang.psd1" -PathType Leaf) {
		...
		Get-ChildItem –Path "$($PSScriptRoot)\langpacks\$($lang)" –Recurse -include "*.psd1" | ForEach-Object {
			$Global:Lang += Import-LocalizedData -FileName $_.BaseName -BaseDirectory $_.DirectoryName
		}
	} else {
...
Function Modules_Import
{
	...
	if ($Import) {
		...
		<#
			...
			.导入所有 *.psd1
		#>
		Get-ChildItem –Path "$($PSScriptRoot)\Functions" –Recurse -include "LXPs.*.psd1" | ForEach-Object {
