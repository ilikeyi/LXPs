...
Function LXPs_Download
{
	...
	{
		$UI_Main_Channel_Menu.Controls | ForEach-Object {
			if ($_ -is [System.Windows.Forms.RadioButton]) {
				if ($_.Enabled) {
					if ($_.Checked) {
						$UI_Main_Channel_Name.Tag = $_.Tag
						Save_Dynamic -regkey "LXPs" -name "Channel" -value $_.Tag
					}
				}
			}
		}
	...
	$UI_Main_Download_Click = {
		if ($UI_Main_Download.Checked) {
			Save_Dynamic -regkey "LXPs" -name "IsDownloadAll" -value "True"
			$UI_Main_Download_Menu.Enabled = $False
			$UI_Main_Download_Match_Version.Enabled = $False
			$UI_Main_Sync_Some_Location.Enabled = $False
			$UI_Main_Sync_Some_Location_Tips.Enabled = $False
		} else {
			Save_Dynamic -regkey "LXPs" -name "IsDownloadAll" -value "False"
	...
	$UI_Main_Download_Match_Version_OK = New-Object system.Windows.Forms.Button -Property @{
		...
		add_Click      = {
			...
			#>
			Save_Dynamic -regkey "LXPs" -name "LXPsSelect" -value $UI_Main_Download_Match_Version_Select.Text
	...
	<#
		.频道
	#>
	$UI_Main_Channel_Menu = New-Object system.Windows.Forms.FlowLayoutPanel -Property @{
		margin         = "0,0,0,30"
		BorderStyle    = 0
		autoSize       = 1
		autoSizeMode   = 1
	...
	$UI_Main_Channel_Name = New-Object system.Windows.Forms.Label -Property @{
		Height         = 30
		Width          = 490
		margin         = "0,10,0,0"
		Text           = $lang.DownloadChannel
		Tag            = "Retail"
	}
	$UI_Main_Channel_Fast = New-Object System.Windows.Forms.RadioButton -Property @{
		Height         = 35
		Width          = 450
		Padding        = "18,0,0,0"
		Tag            = "WIF"
		Text           = $lang.ChannelWIF
		add_Click      = { LXPs_Refresh_Channel }
	}
	$UI_Main_Channel_Slow = New-Object System.Windows.Forms.RadioButton -Property @{
		Height         = 35
		Width          = 450
		Padding        = "18,0,0,0"
		Tag            = "WIS"
		Text           = $lang.ChannelWIS
		add_Click      = { LXPs_Refresh_Channel }
	}
	$UI_Main_Channel_RP = New-Object System.Windows.Forms.RadioButton -Property @{
		Height         = 35
		Width          = 450
		Padding        = "18,0,0,0"
		Tag            = "RP"
	...
	$UI_Main_Channel_Retail = New-Object System.Windows.Forms.RadioButton -Property @{
		Height         = 35
		Width          = 450
		Padding        = "18,0,0,0"
		Tag            = "Retail"
		Text           = $lang.ChannelRetail
		add_Click      = { LXPs_Refresh_Channel }
	}
	...
	$UI_Main_Sync_Some_Location = New-Object System.Windows.Forms.CheckBox -Property @{
		...
		add_Click      = {
			if ($UI_Main_Sync_Some_Location.Checked) {
				Save_Dynamic -regkey "LXPs" -name "IsSyncSaveTo" -value "True"
			} else {
				Save_Dynamic -regkey "LXPs" -name "IsSyncSaveTo" -value "False"
			}
	...
	if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$($Global:Author)\LXPs" -Name "IsSyncSaveTo" -ErrorAction SilentlyContinue) {
		switch (Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$($Global:Author)\LXPs" -Name "IsSyncSaveTo" -ErrorAction SilentlyContinue) {
			"True" {
	...
	$UI_Main_Match_No_Select_Item = New-Object system.Windows.Forms.LinkLabel -Property @{
		...
		add_Click      = {
			...
			if (Test-Path -Path $FolderDirect -PathType Container) {
				$NewGroupRegion = Language_Region
				$Combined = ($NewGroupRegion + $Global:Language_Only_LIP) | ForEach-Object {
					[PSCustomObject]@{
						RegionID = $_.RegionID
						Region   = $_.Region
						Tag      = $_.Tag
				...
				$Region = $Combined | Sort-Object -Property RegionID -Unique | Sort-Object -Property Region
	...
	$UI_Main_Tips_Mask_DoNot = New-Object System.Windows.Forms.CheckBox -Property @{
		...
		add_Click      = {
			if ($UI_Main_Tips_Mask_DoNot.Checked) {
				Save_Dynamic -regkey "LXPs" -name "LXPsTipsWarning" -value "True"
			} else {
				Save_Dynamic -regkey "LXPs" -name "LXPsTipsWarning" -value "False"
			}
		}
	}
	...
	$UI_Main_Channel_Menu.controls.AddRange((
		$UI_Main_Channel_Name,
		$UI_Main_Channel_Fast,
		$UI_Main_Channel_Slow,
		$UI_Main_Channel_RP,
		$UI_Main_Channel_Retail
	))

	if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$($Global:Author)\LXPs" -Name "Channel" -ErrorAction SilentlyContinue) {
		$GetSaveChannel = Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$($Global:Author)\LXPs" -Name "Channel" -ErrorAction SilentlyContinue
	} else {
		Save_Dynamic -regkey "LXPs" -name "Channel" -value "Retail"
		$GetSaveChannel = "Retail"
	}

	$UI_Main_Channel_Menu.Controls | ForEach-Object {
		if ($_ -is [System.Windows.Forms.RadioButton]) {
			if ($GetSaveChannel -eq $_.Tag) {
				$UI_Main_Channel_Name.Tag = $_.Tag
				$_.Checked = $True
			} else {
				$_.Checked = $False
	...
	<#
		...
		.获取语言列表并初始化选择
	#>
	if (-not (Get-ItemProperty -Path  "HKCU:\SOFTWARE\$($Global:Author)\LXPs" -Name 'Select_Download_Language' -ErrorAction SilentlyContinue)) {
		Save_Dynamic -regkey "LXPs" -name "Select_Download_Language" -value "" -Type "MultiString"
	}
	$GetSelectLXPsLanguageRemove = Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$($Global:Author)\LXPs" -Name "Select_Download_Language" -ErrorAction SilentlyContinue
	...
	if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$($Global:Author)\LXPs" -Name "LXPsSelect" -ErrorAction SilentlyContinue) {
		$GetLXPsSelect = Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$($Global:Author)\LXPs" -Name "LXPsSelect" -ErrorAction SilentlyContinue
	...
	if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$($Global:Author)\LXPs" -Name "IsDownloadAll" -ErrorAction SilentlyContinue) {
		switch (Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$($Global:Author)\LXPs" -Name "IsDownloadAll" -ErrorAction SilentlyContinue) {
			"True" {
	...
	if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$($Global:Author)\LXPs" -Name "LXPsTipsWarning" -ErrorAction SilentlyContinue) {
		switch (Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$($Global:Author)\LXPs" -Name "LXPsTipsWarning" -ErrorAction SilentlyContinue) {
			"True" {
				$UI_Main_Tips_Mask.Visible = $False
...
function LXPs_URL_Download_Process
{
	...
	write-host "  $($lang.DownloadChannel): " -NoNewline -ForegroundColor Yellow
	switch ($UI_Main_Channel_Name.Tag) {
		"WIF" {
			Write-Host $lang.ChannelWIF -ForegroundColor Green
		}
		"WIS" {
			Write-Host $lang.ChannelWIS -ForegroundColor Green
		}
		"RP" {
			Write-Host $lang.ChannelRP -ForegroundColor Green
		}
		Default {
			Write-Host $lang.ChannelRetail -ForegroundColor Green
		}
	}
	...
	try {
		...
		$myParameters = "type=url&url=$($NewStoreURL)&ring=$($UI_Main_Channel_Name.Tag)"
